...
import { resolveSiteLogoUrl } from "@/lib/site-logo";
...
export default function HeaderBarClient({
  ...
  const [logoUrl, setLogoUrl] = useState<string>(() =>
    resolveSiteLogoUrl(initialLogoUrl)
  );
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.logoUrl?.trim()) {
            setLogoUrl(resolveSiteLogoUrl(data.logoUrl));
          }
  ...
  return (
    <header className="px-2 sm:px-4 py-4 flex items-center justify-center sticky top-0 z-[120] transition-all duration-300">
      <div className="w-full max-w-7xl relative">
        ...
        <div className="flex items-center gap-3 sm:gap-4 rounded-3xl bg-gradient-to-r from-white/96 via-white/92 to-white/96 dark:from-slate-900/95 dark:via-slate-800/95 dark:to-slate-900/95 backdrop-blur-2xl border border-white/40 dark:border-white/10 shadow-[0_8px_32px_-12px_rgba(0,0,0,0.25)] dark:shadow-[0_8px_32px_-12px_rgba(0,0,0,0.6)] px-3 sm:px-6 py-2.5 sm:py-3 ring-1 ring-[color:var(--primary)]/15 hover:ring-[color:var(--primary)]/25 transition-all duration-300 group">
          <a
            href={homeBase}
            className="flex shrink-0 items-center py-1 pr-2 sm:pr-3 group hover:opacity-90 transition-opacity"
            aria-label={t.app_name}
          >
            <SiteLogo src={logoUrl} priority key={logoUrl} />
          </a>
