  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // loading reste true jusqu'à la fin du try (connexion Supabase + redirection)
    ...
    <div className="w-full max-w-2xl bg-white rounded-2xl border border-black/10 p-6 shadow-md">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <button
          ...
          disabled={loading}
          aria-busy={loading}
          className="w-full h-11 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-semibold transition-colors disabled:opacity-90 disabled:cursor-wait flex items-center justify-center gap-3"
        >
          {loading ? (
            <>
              <span
                className="inline-block size-5 shrink-0 rounded-full border-2 border-white/35 border-t-white animate-spin motion-reduce:animate-none"
                aria-hidden
              />
