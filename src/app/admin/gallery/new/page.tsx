    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 py-10 w-full">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/gallery" className="text-sm rounded-full border border-black/15 px-3 h-9 inline-flex items-center hover:bg-black/5">← {locale === "fr" ? "Retour" : "Back"}</Link>
        </div>
        <div className="mt-6 rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
          <form action="/api/admin/gallery" method="post" className="grid gap-4" encType="multipart/form-data">
            ...
            <div className="grid gap-2 text-sm">
              ...
              <div id="dropZone" className="relative h-48 rounded-lg border border-dashed border-black/25 flex flex-col items-center justify-center text-xs text-black/60 cursor-pointer bg-black/[0.02] hover:bg-black/[0.04] transition">
                <input id="fileInput" name="imageFile" type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" required />
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Link href="/admin/gallery" className="rounded-full h-10 px-4 border border-black/15 bg-white hover:bg-black/5">{locale === "fr" ? "Annuler" : "Cancel"}</Link>
